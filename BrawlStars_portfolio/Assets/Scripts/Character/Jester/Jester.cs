...
public class Jester : Hero
{
    ...
    IEnumerator BulletInit()
    {
        ...
        if (m_bCheckStart)
        {  /*
            Vector3 Bojung = m_tfResultTarget.position - m_tBulletPos[0].position;
            Vector3 yBojung = new Vector3(Bojung.x, 0, Bojung.z);
            yBojung.Normalize();
            Vector3 Bojung_1 = m_tfResultTarget.position - m_tBulletPos[1].position;
            Vector3 yBojung_1 = new Vector3(Bojung.x, 0, Bojung.z);
            yBojung.Normalize();
            Vector3 Bojung_2 = m_tfResultTarget.position - m_tBulletPos[2].position;
